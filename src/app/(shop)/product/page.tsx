...
export default function Products() {
  ...
  const { data: recomendationProducts, isLoading: isLoadingRecomendation } =
      ...
      <div className="w-content flex pt-5 gap-6">
        <div className="flex-[1] border border-gray-300 rounded-xl py-6 px-4 h-fit">
          ...
          <FilterPrice
            ...
            onChange={(value) => {
              if (
                value.min !== parseInt(searchParams?.get("min_price") as string)
              ) {
                handleChangeFilter("min_price", `${value.min}`);
              } else {
                handleChangeFilter("max_price", `${value.max}`);
              }
            }}
          />
          <div className="w-full separator my-4" />
